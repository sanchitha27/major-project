...
import { generateReactionLogic, checkReactionTrigger } from '@/lab/data/reactionLogic'
...
const LabEquipment = forwardRef<any, LabEquipmentProps>(function LabEquipment({ experiment, onStepComplete, onChemicalAdd, onReactionTrigger }, ref) {
  ...
  const handleChemicalPour = (chemical: Chemical) => {
    ...
    // Special case for Potassium Permanganate Reduction Reaction experiment
    if (experiment.id === 'potassium-permanganate-reduction') {
      ...
      if (hasKMnO4 && isOxalicAcid && !hasH2SO4) {
        console.log('⚠️ Warning: Add acid first to allow reduction to occur.')
        // We could show a warning to the user here
        // For now, we'll just log it and continue
      }
    }
